<template>
  <n-card :bordered="false" class="proCard">
    <BasicTable title="接口管理" titleTooltip="" :columns="columns" :request="loadDataTable" :row-key="(row) => row.id"
...
const loadDataTable = async (res) => {
  return await api.getDataList({ ...res });
};
...
function newData() {
  router.push({ name: 'case_ui_element_edit', params: { id: 0 } });
}
...
function handleEdit(record) {
  // 跳转编辑页面
  router.push({ name: 'case_ui_element_edit', params: { id: record.id } });
}
</script>
