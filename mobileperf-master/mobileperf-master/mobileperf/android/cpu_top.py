...
class CpuCollector(object):
    ...
    def _collect_package_cpu_thread(self,start_time):
        ...
        try:
            ...
            logger.error(e)
        while  not self._stop_event.is_set() and time.time() < end_time:      #not self._stop_event.is_set()使用这个不会停下来，换一个看看能不能停下来
            try:
...
class CpuMonitor(object):
    ...
    cpu 监控器
    '''
    def __init__(self, device_id, packages, interval=5,timeout=18):
...
if __name__ == "__main__":
    ...
    monitor = CpuMonitor("2f671c2b",["com.camelgames.aoz.debuglz4"], 1)
