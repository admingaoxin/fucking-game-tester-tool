...
class MemInfoDevice:
    ...
    def _parse(self):
        ...
        for package in self.packages:
            ...
            RE_PROCESS_MEMORY_2 = re.compile(r"([\d,]+)\s+kB:\s+\d+\s+kB:\s+" + package + r"\s+\(pid\s+(\d+)")
...
class MemInfoPackageCollector(object):
    ...
    def _collect_memory_thread(self, start_time):
        dumpheap_freq = RuntimeData.config_dic.get("dumpheap_freq", 60)
...
if __name__ == "__main__":
    ...
    monitor = MemMonitor("2f671c2b",["com.camelgames.aoz.debuglz4"],5)
