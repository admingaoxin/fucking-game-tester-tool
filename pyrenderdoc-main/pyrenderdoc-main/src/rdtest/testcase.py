...
class ShaderVariableCheck:
    def __init__(self, var: rd.ShaderVariable, name: str):
        self.var = var
        _print(f"{type(var)} ShaderVariable {var.name} [{var.rows}, {var.columns}]  arraySize {len(self.var.members)} ")
    ...
    def raw_value(self, count,Var= None):
        vals = []
        if self.var.type == rd.VarType.Float:
            itemSize  = 4
            vals = list(self.var.value.f32v[0:count])
            ...
            itemSize  = 2
            vals = list(self.var.value.f16v[0:count])
        elif self.var.type == rd.VarType.UInt or self.var.type == rd.VarType.Bool:
            itemSize  = 4
            vals = list(self.var.value.u32v[0:count])
        elif self.var.type == rd.VarType.ULong:
            itemSize  = 8
            ...
            assert False,f"self.var.type {self.var.type} count {count}  {self.var.value.s8v[0:count]}  {list(self.var.value.u32v[0:count])}  vals {vals}"
        if Var:
    ...
    def value_recurssive(self):
        ...
        def rec(var,d):
            for i, m in enumerate(var.members):
                _print(f"member {type(m)} {m.name} type {m.type.name} flags {m.flags} value {m.value}  [{m.columns},{m.rows}] members {len(m.members)}")
                block_size = m.columns * m.rows
                if block_size==0:
                    d[m.name] = {}
                    rec(m, d[m.name])
                    #for j, m2 in enumerate(m.members):
                    #    _print(f"member2 {type(m2)} {m2.name} type {m2.type.name} flags {m2.flags} value {m2.value}  [{m2.columns},{m2.rows}] members {len(m2.members)}")
                    #    block_size = m2.columns * m2.rows
                        #assert len(m2.members) == 0,f" len(m.members) != 0  => {len(m.members)}"
                    #    d.append(ShaderVariableCheck(m2, m2.name).raw_value(block_size))
                else:
                    assert len(m.members) == 0,"len(m.members) != 0"
                    Var = _Var()
                    Var.itemSize   = 0
                    Var.cols       = m.columns
                    Var.rows       = m.rows
                    Var.arraySize  = 1 if len(self.var.members) == 0 else len(self.var.members)
                    Var.size       = Var.cols*Var.rows*Var.arraySize*Var.itemSize
                    Var.data       = ShaderVariableCheck(m, m.name).raw_value(block_size,Var)
                    #print(f"{m.name}  {Var.data}")
                    d[m.name] = Var
        rec(self.var,d)
