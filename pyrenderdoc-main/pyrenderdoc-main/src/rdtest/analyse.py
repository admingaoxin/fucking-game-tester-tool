...
from .logging import TestFailureException
...
def format_print(fmt:rd.ResourceFormat):
    print(f"type {ResourceFormatType[fmt.type]}")
    print(f"compCount {fmt.compCount}")
    print(f"compByteWidth {fmt.compByteWidth}")
    print(f"compType {CompType[fmt.compType]}")

def raw_reshape(d,fmt):
    if fmt.type ==  rd.ResourceFormatType.R10G10B10A2:
        itemBytes = 4
    elif fmt.type == rd.ResourceFormatType.R11G11B10:
        itemBytes = 4
    elif fmt.type == rd.ResourceFormatType.R5G6B5:
        itemBytes = 4
    elif fmt.type == rd.ResourceFormatType.R9G9B9E5:
        itemBytes = 4
    elif fmt.type == rd.ResourceFormatType.R4G4B4A4:
        itemBytes = 2
    elif fmt.type == rd.ResourceFormatType.R4G4:
        itemBytes = 1
    elif fmt.type == rd.ResourceFormatType.D16S8:
        itemBytes = 3
    elif fmt.type == rd.ResourceFormatType.D24S8:
        itemBytes = 4
    elif fmt.type == rd.ResourceFormatType.D32S8:
        itemBytes = 5
    elif fmt.type == rd.ResourceFormatType.S8:
        itemBytes = 1
    elif fmt.type == rd.ResourceFormatType.A8:
        itemBytes = 1
    elif fmt.type == rd.ResourceFormatType.YUV8:
        itemBytes = 1
    else:
        format_print(fmt)
        check(False,f"NIL")

    return [d[i*itemBytes:(i+1)*itemBytes] for i in range(len(d)//itemBytes)]
...
def unpack_data(fmt: rd.ResourceFormat, data: bytes, data_offset: int):
    ...
    if fmt.Special():
        return  raw_reshape(data[data_offset:],fmt)
    ...
    if data_offset >= len(data):
        if len(data) == 0:
            return None
        format_print(fmt)
        check(False,f"Out of range! {data_offset} >= {len(data)} { CompType[fmt.compType]} ")
        ...
        if striprestart_index is None or idx != striprestart_index:
            for attr in attrs:
                ...
                if attr.mesh.instanced:
                    offset = (attr.mesh.vertexByteStride*instance + attr.mesh.vertexByteStride * int(instance / max(attr.mesh.instStepRate, 1)))

                _print(f"i {i} idx {idx} instance  {instance}  attr.mesh.instStepRate {attr.mesh.instStepRate}  attr.name {attr.name} attr.mesh.vertexByteOffset {attr.mesh.vertexByteOffset} + offset  {offset} - buffer_ranges[attr.mesh.vertexResourceId] {buffer_ranges[attr.mesh.vertexResourceId]} ")
